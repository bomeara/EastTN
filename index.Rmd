...
As of `r tail(tn_death_history$target_end_date,1)`, `r tail(tn_death_history$value,1)` people have died from covid in Tennessee. Here are the predictions over the next few weeks, from https://covid19forecasthub.org/ (which aggregates multiple models): 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
tn_death_predictions_point <- subset(tn_death_predictions, type=="point")

tn_death_prediction_organized <- data.frame(Date=tn_death_predictions_point$target_end_date,`Expected Deaths`=tn_death_predictions_point$value, Range=rep(NA, nrow(tn_death_predictions_point)))
