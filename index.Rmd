...
Currently, at least 1 of every `r round(100/current_active_region)` people in our region (`r round(current_active_region,2)`%) have *active covid infections* (based on the tests reported to the state; additional people may have covid but have not been tested). Another way of looking at this: if UTK's Neyland Stadium had this percentage of people with active covid infections, `r format(round(0.01*current_active_region*neyland_capacity),big.mark=",",scientific=FALSE)` people in the 102,455 person stadium would have covid right now (though hopefully people who know they are sick stay home). Over the course of the pandemic, `r format(round(0.01*current_percent_dead_region*neyland_capacity),big.mark=",",scientific=FALSE)` people in a full stadium would have died from covid, `r format(round(0.01*current_percent_hospitalized_region*neyland_capacity),big.mark=",",scientific=FALSE)` would have been hospitalized, and `r format(round(0.01*current_percent_confirmed_region*neyland_capacity),big.mark=",",scientific=FALSE)` would have had a case of covid.
...
cdc_new_plot <- ggplot(cdc_local, aes(x=Date, y=Cases_7_days_per_100K_County, color=`Community Level`)) + scale_color_manual(values=c("mediumseagreen", "khaki1", "red")) + geom_point() + ylab("Seven day number of covid cases per 100K people") + xlab("Date") + facet_wrap(~ County, ncol=1) + geom_hline(yintercept=500, linetype="dashed") 
print(cdc_new_plot)
```


Here are the percent of tests positive, averaged over a week. Since data are released with a lag, the dark gray rectangle shows the region from the last released data point to the present. 

```{r percentagpositive, echo=FALSE, message=FALSE, warning=FALSE, fig.alt="Percentage of positive tests over time"}

positivity <- ggplot(daily_focal, aes(x=DATE, y=PositivityPercentage_per_week, group=Region)) + geom_line(aes(colour=Region)) + ylab("Percent of positive tests reported to state") + xlab("Date") + ylim(0,NA)  + scale_colour_viridis_d(end=0.8) + theme_light() + theme(legend.justification = c(0, 1), legend.position = c(0.1, .9)) + geom_rect(mapping=aes(xmin=24*60*60+max(DATE), xmax=Sys.time(), ymin=0, ymax=50))

print(positivity)
```


Here are the deaths over time associated with covid in East TN (excluding Dec 22, 2021, when there seems to be an error in the state's death records). Since data are released with a lag, the dark gray rectangle shows the region from the last released data point to the present. 
...
try(daily_death_plot <- ggplot(daily_focal_easttn, aes(x=DATE, y=NEW_DEATHS)) + theme_light() + geom_ma(aes( linetype="a"), n=7) +  guides(linetype = "none") + ylab("Deaths per day (7 day average)") + xlab("Date") + ggtitle("Covid-Associated Deaths in East TN") + geom_rect(mapping=aes(xmin=24*60*60+max(DATE), xmax=Sys.time(), ymin=0, ymax=50)) + ylim(0,55))
try(print(daily_death_plot))

```


Here are the percent of people who have active covid cases right now, who have tested and gotten positive tests, *and* who have reported this to the state. Since data are released with a lag, the dark gray rectangle shows the region from the last released data point to the present. 
...
active <- ggplot(daily_focal, aes(x=DATE, y=PercentActive, group=Region)) + geom_line(aes(colour=Region)) + ylab("Percent of people with active covid infections each day") + xlab("Date") + ylim(0,NA)  + scale_colour_viridis_d(end=0.8) + theme_light() + theme(legend.justification = c(0, 1), legend.position = c(0.1, .9)) + geom_rect(mapping=aes(xmin=24*60*60+max(DATE), xmax=Sys.time(), ymin=0, ymax=3))
