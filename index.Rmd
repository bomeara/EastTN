...
We can also plot using the [new (Feb 2022) guidelines from the CDC](https://www.cdc.gov/coronavirus/2019-ncov/your-health/covid-by-county.html) for low, medium, high community level; I've also recalculated past data so we can see how it would apply then. Note that though I plot case numbers, which are used by the CDC among other metrics, these might not be the most informative given the current state of testing; the hospitalization data (see tab above) could be useful.
