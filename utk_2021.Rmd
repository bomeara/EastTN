...
local_cases <- data.frame(DATE=c(zukowski_data$DATE,knox17to25$DATE), Cases=c(zukowski_data$CASES_NEW, knox17to25$NEW_CASES), Population=c(rep("UTK all", nrow(zukowski_data)), rep("Knox County age 17-25", nrow(knox17to25))))
local_cases_plot <- ggplot(local_cases, aes(x=DATE, y=Cases, group=Population, colour=Population)) +  geom_line() + ylab("Number of new cases per day") + xlab("Date") + ylim(0,NA)  + scale_colour_viridis_d(end=0.8) + theme_classic() 
