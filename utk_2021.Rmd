...
Note that UTK's data is released [here](https://www.utk.edu/coronavirus/guides/data-monitoring-and-contingency-options/), but it requires manual download; I am taking advantage of [Alex Zukowski's website](https://sites.google.com/view/utk-covid19/data?authuser=0) to get that compilation of the data. Citation: Zukowski, Taylor A. “University of Tennessee, Knoxville COVID-19 Dataset.” COVID-19 in The University of Tennessee, Knoxville, 2020, sites.google.com/view/utk-covid19/data
...
local_cases_plot <- ggplot(local_cases, aes(x=DATE, y=Cases, group=Population, colour=Population)) +  geom_line() + ylab("Number of new cases per day (7 day average)") + xlab("Date") + ylim(0,NA)  + scale_colour_viridis_d(end=0.8) + theme_light()
