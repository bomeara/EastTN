...
list(
  ...
  #tar_target(cases_by_county, GetCountyCaseAverages()),
  #tar_target(countycitystate, GetCountyCityState()),
  #tar_target(hospitalization_by_county, GetHospitalData(countycitystate)),
  #tar_target(aggregation_by_county, AggregateCountyInformation(hesitancy_by_county, cases_by_county, hospitalization_by_county))
  ...
  tar_target(county_pages, RenderCountyPages(site_info_formatted, nationwide_info, aggregation_by_county, date_community=cdc_weekly_metadata$report_date, all_predictions=all_predictions, all_time_series=all_time_series)),
  tar_invalidate("report"),
  #tar_invalidate("county_pages"),
  tar_invalidate("cdc_weekly_metadata"),
