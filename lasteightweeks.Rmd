...
active <- ggplot(subset(daily_focal, daily_focal$DATE >= daily_focal$DATE[nrow(daily_focal)]-elapsed_seconds), aes(x=DATE, y=PercentActive, group=Region)) + geom_line(aes(colour=Region)) + ylab("Percent of people with active covid infections each day") + xlab("Date") + ylim(0,NA)  + scale_colour_brewer(palette = "Dark2") + theme_light()
...
schools <- plyr::rbind.fill(schools_knox_percent_for_focal, schools_oakridge_for_focal)


active <- ggplot(subset(schools, schools$DATE >= schools$DATE[nrow(schools)]-elapsed_seconds), aes(x=DATE, y=PercentActive, group=Region)) + geom_line(aes(colour=Region)) + ylab("Percent of people with active covid infections each day") + xlab("Date") + ylim(0,NA)  + scale_colour_brewer(palette = "Dark2") + theme_light()
